...
export default function Home() {
 ...
 const filteredLabs = selectedTestType ? labs.filter(lab => lab.test_type === selectedTestType) : labs;
 ...
 const chartData = filteredLabs.map((lab) => ({
  ...
  // Unique Test Types
  const uniqueTestTypes = [...new Set(labs.map(lab => lab.test_type))];

  const chart = (
    <ResponsiveContainer width="100%" aspect={1}>
      <LineChart data={chartData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
        <CartesianGrid strokeDasharray="3 3" />
        <XAxis dataKey="test_type" label={{ value: 'Date', position: 'insideBottomRight', offset: -5 }} scale="time" domain={['auto', 'auto']} tickFormatter={(unixTime) => moment(unixTime).format('YYYY-MM-DD')} />
  ...
  return (
    <div className="font-sans min-h-screen bg-gray-900 py-6 flex flex-col justify-center sm:py-12 text-white">
      ...
      <div className="relative py-3 sm:max-w-xl sm:mx-auto">
        <h1 className="text-2xl text-center font-semibold tracking-wide mt-6 mb-6 text-blue-400">Labs</h1>
